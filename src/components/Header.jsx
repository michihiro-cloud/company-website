  ...
  return (
    <header className="relative border-b border-slate-800 bg-gradient-to-b from-slate-950 via-slate-950 to-slate-900 shadow-[0_24px_60px_-24px_rgba(0,0,0,0.5)]">
      ...
      <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
        ...
        <nav className="mt-3 flex gap-5 overflow-x-auto pb-1 md:hidden">
          {navItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              className={({ isActive }) =>
                `whitespace-nowrap text-sm font-medium transition ${isActive ? 'text-brand' : 'text-slate-400 hover:text-slate-100'}`
              }
            >
              {item.label}
            </NavLink>
